...
function getPatterns() {
  return [
    ...
    },
    {
      ...
      handler: async (req: NextRequest, res: NextResponse) => {
        console.log('Middleware: Handling /home route');
        
        const supabase = createMiddlewareClient(req, res);
        const { data, error } = await supabase.auth.getSession();

        console.log('Middleware: Session data:', { data, error });

        if (error || !data.session) {
          console.log('Middleware: No session, redirecting to sign in');
          return NextResponse.redirect(
            new URL('/auth/sign-in', req.nextUrl.origin).href,
          );
        }
        ...
        if (requiresMultiFactorAuthentication) {
          console.log('Middleware: MFA required, redirecting to verify');
          return NextResponse.redirect(
            new URL(pathsConfig.auth.verifyMfa, origin).href,
          );
        }

        console.log('Middleware: User authenticated, proceeding to /home');
      },
    },
  ];
}
